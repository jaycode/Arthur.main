...
from app_workspace.mod_cmd.helpers import get_package_module_pairs, get_base_package_module_pairs, get_package
...
def get_docs(cmd = None):
    """Get all the available modules and return their documentations. This method will find scripts
       inside another directory under `app_workspace/commands` directory.
    """
    docs = []
    if cmd is None:
        last_package = ''
        for package, module in chain(get_package_module_pairs(), get_base_package_module_pairs()):
            ...
            if package != '':
                package += '.'
            mod = importlib.import_module("app_workspace.mod_cmd.commands.%s%s" % (package, module))
            docs.append(("\t%s: %s" % (module, mod.run.__doc__.split('\n', 1)[0].strip())))
    else:
        package = get_package(cmd)
        if package != '':
            package += '.'
        mod = importlib.import_module("app_workspace.mod_cmd.commands.%s%s" % (package, cmd))
